...
slider_prim = "/World/slider_1"
slider_name = "my_slider"
slider_init_position = np.array([0.0, 0.0, 0.0])
slider_init_orientation = np.array([0.0, 0.0, 0.0])
slider_init_scale = np.array([0.01, 0.01, 0.01])
my_task = SliderTask(prim_path = slider_prim, 
                        name = slider_name, 
...
while simulation_app.is_running():
    ...
    simulation_app.update()
    my_task._slider_node.publish_all()
    ...
    for i in range(10000):  # forget how to set this time interval
        if i%100 == 0:  # change to 1000, faster, but too fast
            my_task._slider_node.update_test()
