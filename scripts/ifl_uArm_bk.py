...
uarm_prim = "/World/uarm_1"
uarm_name = "my_uArm"
uarm_init_position = np.array([0.0, 0.0, 0.0])
uarm_init_orientation = np.array([0.0, 0.0, 0.0])
uarm_init_scale = np.array([1, 1, 1])
my_task = UarmTask(prim_path = uarm_prim, 
                        name = uarm_name, 
                        usd_path = uarm_model,
...
my_cube_initial_position = np.array([0.2, 0.0, 0.025]) / get_stage_units()
...
while simulation_app.is_running():
    ...
    for i in range(10000):  # forget how to set this time interval
        if i%100 == 0:  # change to 1000, faster, but too fast
            my_task._uarm_node.update_test()
